...
from .cell_rule_execution import CellExecutionResult
...
def build_action_focused_summary(
    ...
    *,
    ...
    for (field, role, entry), group in unclassified.groupby(["input_variable", "matched_role", "entry"], dropna=False):
        rows.append({
            "priority": "HIGH", "user_decision": "RAISE_ISSUE_FOR_RCA", "scope": "UNCLASSIFIED_EVIDENCE",
            ...
            "recommended_next_step": "Raise an RCA on input-domain quality and declaration coverage before accepting these rows.",
            "rca_warranted": True, "evidence_reference": f"assessment_ledger:{entry}",
        })
