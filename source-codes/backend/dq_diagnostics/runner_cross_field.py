...
from .engines.base import Readiness, register_engine
from .engines.cross_field import ENGINE_VERSION, iter_run, render_pdf, render_text
from .engines.cross_field.result import StructuredResult
from .readiness import readiness as readiness_fn
...
def run_results(run_id: str) -> dict[str, Any]:
    """Decision-type-shaped results + findings for one run (the Findings pane)."""
    run_row = manifest_mod.get_run(run_id)
    results = s.query("diag_results", run_id=run_id)
    out = []
    for res in results:
        if run_row["diagnostic_id"] == 2 and (res.get("metrics_json") or {}).get("result_kind") == "feature":
            from .binning_reviews import hydrate_result
            res = hydrate_result(res)
        findings = s.query("diag_findings", result_id=res["result_id"], order_by="seq")
        for f in findings:
            f["dispositions"] = s.query("diag_dispositions", target_type="finding",
                                        target_id=f["finding_id"], order_by="id")
            # Issue lifecycle is part of the shared finding projection for
            # every diagnostic. Diagnostic 2 additionally reconciles an older
            # promoted issue for the same feature, even when it came from a
            # previous immutable run/finding.
            issue_rows = s.execute(
                "SELECT * FROM issues_v2 WHERE finding_id=? "
                "AND COALESCE(status, '') <> 'Superseded' "
                "ORDER BY created_at, issue_row_id LIMIT 1", [f["finding_id"]])
            issue = issue_rows[0] if issue_rows else None
            if not issue and run_row["diagnostic_id"] == 2:
                from .runner_feature_target import existing_candidate_issue
                issue = existing_candidate_issue({**f, "item_id": run_row["item_id"]})
            if issue:
                f["existing_issue"] = {
                    "issue_row_id": issue["issue_row_id"],
                    "finding_id": issue.get("finding_id"),
                    "status": issue.get("status"),
                    "same_finding": issue.get("finding_id") == f["finding_id"],
                }
        out.append({**res, "findings": findings})
    return {
        "run": {k: run_row[k] for k in ("run_id", "item_id", "diagnostic_id", "status",
...
def coverage_summary(item_id: str) -> dict[str, Any]:
    ...
    """
    from .register import coverage_map, list_register, scoring_weights
