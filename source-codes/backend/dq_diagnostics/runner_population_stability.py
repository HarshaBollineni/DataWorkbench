...
from .engines.population_stability import calculate_feature_psi, split_population, validate_bin_definition
from .manifest import DONE, DRAFT, RUNNING
from . import manifest_population_stability as manifest_mod
...
def run_results(run_id: str) -> dict[str, Any]:
    ...
    from .engines.population_stability.engine import bin_display_labels
    repo = AnalysisArtifactRepository()
    for row in response.get("results") or []:
        metrics = row.get("metrics_json") or {}
        if metrics.get("result_kind") != "psi_feature" or not metrics.get("bin_artifact_id"):
            continue
        try:
...
def report_text(run_id: str) -> str:
    payload = run_results(run_id)
    return f"Population Stability Index — run {run_id}\nContextual results: {len(payload.get('results', []))}\n"
