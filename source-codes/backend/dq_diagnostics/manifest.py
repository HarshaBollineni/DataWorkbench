...
from .engines.cross_field import ENGINE_VERSION
from .engines.cross_field.roles import RoleMatch, derive_vocabulary, resolve_roles
from .engines.cross_field.rules import Rule
...
class ManifestError(RuntimeError):
    """A manifest operation that cannot proceed (wrong state, unknown field)."""
...
def get_run(run_id: str) -> dict[str, Any]:
    row = s.query_one("diag_runs", run_id=run_id)
    if not row:
...
def record_decision(run_id: str, kind: str, payload: dict[str, Any], actor: str) -> dict[str, Any]:
    """Append-only decision record (CFR-12 / PLT-05)."""
    ts = s.now_ist()
    row_id = s.insert("diag_run_decisions", {
        "run_id": run_id, "kind": kind, "payload_json": payload, "actor": actor, "ts": ts,
    })
    return {"id": row_id, "run_id": run_id, "kind": kind, "payload": payload,
            "actor": actor, "ts": ts}


def list_decisions(run_id: str) -> list[dict[str, Any]]:
    return s.query("diag_run_decisions", run_id=run_id, order_by="id")
