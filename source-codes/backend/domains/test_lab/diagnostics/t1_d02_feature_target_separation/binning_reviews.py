...
def hydrate_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if artifact_ids.get("roc_feature"):
            roc_meta, roc_payload = repo.get(artifact_ids["roc_feature"])
            enriched["roc_detail"] = roc_payload.get("result", roc_payload)
            profile_source_ids = roc_meta.source_artifact_ids
        elif artifact_ids.get("fine_bins"):
            profile_source_ids = repo.get_metadata(
                artifact_ids["fine_bins"]).source_artifact_ids
        for source_id in profile_source_ids:
            source_meta = repo.get_metadata(source_id)
            if (source_meta.artifact_type == "column_profile"
                    and source_meta.feature == metrics.get("feature")):
                _profile_meta, enriched["data_profile"] = repo.get(source_id)
                enriched["profile_artifact_id"] = source_id
                break
...
def create_numeric_override(result_id: str, cuts: str, special_values: str = "",
    ...
    from domains.test_lab.shared.binning.binning import fit_feature_binning
