# T1-D02 Single-feature target separation backend

This package owns the production T1-D02 adapter, ROC/Gini engine, role selection, orchestration,
manifest, runner, and diagnostic-specific binning review. Information Value/binning primitives live
under `domains/test_lab/shared/binning` because T4-D14 also consumes them. Former
`dq_diagnostics` module paths remain compatibility aliases during staged migration.
...
After a completed run, Test Lab presents a concise read-only evidence dialog for issue review.
Fine-bin inspection, coarse-bin editing and universal promotion are retained in the RCA source-
evidence workspace after explicit issue promotion; they are not routine Test Lab review actions.
...
## Governed final report

Completed runs expose PDF download and text preview using the shared diagnostic report route.
The report is rendered from an immutable `feature_target_separation_report` AAR artifact whose
sources are the retained feature-level ROC and IV artifacts. It records scope, thresholds,
rollups, variable metrics, category rationale, issue-review rationale, finding dispositions,
recommended actions and interpretation limits. Category rationale and issue rationale remain
separate: a strong separation category does not by itself prove leakage, and weak univariate
separation does not prove a feature is unusable in combination with other information.

This diagnostic has no AI or semantic-adjudication component, so the report contains no AI
assistance section.
