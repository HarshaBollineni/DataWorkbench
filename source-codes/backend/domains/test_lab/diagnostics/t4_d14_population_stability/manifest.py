...
from domains.test_lab.shared.run_state import DRAFT, RUNNING, ManifestError, get_run, record_decision
...
def _generated_payload(manifest: dict[str, Any], metadata: dict[str, Any],
    ...
    from .bins import create_draft_bins
    ...
    if route["workflow"] == "generate_constant_guard":
        ...
        from domains.test_lab.shared.binning import BinningConstraints, fit_feature_binning
...
def _prepared_match_payload(manifest: dict[str, Any], metadata: dict[str, Any],
    ...
    from domains.test_lab.shared.binning.models import BinDefinition
...
def generate_bin_draft_events(run_id: str, actor: str = "system", *, max_workers: int = 3):
    ...
    try:
        ...
        if target_aware:
            from domains.test_lab.shared.binning import BinningConstraints
...
def _baseline_exact_categorical_foundation(frame: pd.DataFrame, feature: str,
    ...
    from domains.test_lab.shared.binning.binning import _coarse_groups
    from domains.test_lab.shared.binning.models import BinDefinition
...
def _matched_bin_applicability(manifest: dict[str, Any], feature: str, meta: Any,
    ...
    if outcome.get("fine_definition") and outcome.get("coarse_definition") and not outcome.get("coarse_groups"):
        from domains.test_lab.shared.binning.binning import review_cached_fine_binning
        from domains.test_lab.shared.binning.models import BinDefinition, BinRow
...
def _preview_fine_revision(manifest: dict[str, Any], feature: str, source_meta: Any,
    ...
    from domains.test_lab.shared.binning.binning import _coarse_groups, review_cached_fine_binning
    from domains.test_lab.shared.binning.models import (
