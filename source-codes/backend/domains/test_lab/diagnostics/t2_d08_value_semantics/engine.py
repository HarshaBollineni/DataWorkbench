"""Production cell-level executor for the T2-D08 Value Semantics KB.
...
def _frequency_window(value: Any, period_sample: Any, fallback: int) -> int:
    """Translate a declared update/review cycle into an observation window.

    A cycle needs its two boundary observations, hence ``steps + 1``.  This
    avoids the experiment's former behavior where the declarations were
    checked for presence but the unrelated generic variance window was used.
    """
    if isinstance(value, (int, float)) and int(value) >= 1:
        return max(2, int(value) + 1)
    token = str(value or "").strip().lower()
    aliases = {
        "monthly": "1 month", "quarterly": "1 quarter",
        "semiannual": "6 months", "semi-annual": "6 months",
        "annual": "12 months", "annually": "12 months", "yearly": "12 months",
    }
    normalized = aliases.get(token, token)
    match = re.fullmatch(r"\s*(\d+)\s*(months?|quarters?|years?)\s*", normalized, re.I)
    if not match:
        return max(2, int(fallback))
    count, unit = int(match.group(1)), match.group(2).lower()
    quarterly = "Q" in str(period_sample).upper()
    if unit.startswith("year"):
        steps = count * (4 if quarterly else 12)
    elif unit.startswith("quarter"):
        steps = count if quarterly else count * 3
    else:
        steps = max(1, (count + 2) // 3) if quarterly else count
...
class _Executor:
    ...
    def required(self, rule: Mapping[str, Any], entry: Mapping[str, Any], target: str,
        ...
        # Indicator roles are qualified one-column execution inputs.  Both a
        # missing binding and an ambiguous multi-column binding are therefore
        # UNSCOPED; neither may fall through to ``data[None]`` or silently use
        # whichever column happened to be encountered first.
        missing_roles = [role for role in roles if len(self.roles.get(role, [])) != 1]
    ...
    def unfinished_outcome(self, rule, entry, target):
        if not self.required(rule, entry, target, list(entry.get("indicator_roles", [])),
    ...
    def _declared_variance_window(self, entry: Mapping[str, Any], target: str) -> int:
        period_col = _single(self.roles, "period")
        sample = next((value for value in self.data[period_col] if _is_populated(value)), "") if period_col else ""
        fallback = int(self.declarations.get("variance_window", 3))
        if entry["assessment_grain"] == "segment_and_declared_review_cycle":
            declaration = self.declarations.get("lower_frequency_fields_and_review_cycles") or {}
            value = declaration.get(target) or declaration.get(entry["target_role"]) if isinstance(declaration, Mapping) else declaration
            return _frequency_window(value, sample, fallback)
        if entry["assessment_grain"] == "entity_and_declared_update_cycle":
            return _frequency_window(
                self.declarations.get("behavioural_balance_update_frequency"), sample, fallback,
            )
        return max(2, fallback)

    def _segment_variance(self, rule, entry, target, base_mask: pd.Series,
    ...
    def _entity_variance(self, rule, entry, target, base_mask: pd.Series,
                         window: int) -> pd.Series:
    ...
    def stale(self, rule, entry, target):
        ...
        if not self.required(rule, entry, target, roles, declarations):
            if target in self.sentinels:
                self.plan.append({
                    "input_variable": target, "matched_role": entry["target_role"],
                    "rule": rule["rule"], "entry": entry["entry"],
                    "assessment_grain": "row_and_cell_sentinel_only",
                    "routing_status": "READY_FOR_EVALUATION",
                    "missing_roles": "", "missing_declarations": "",
                })
                self.add_assessments(
                    rule, entry, target, sentinel_mask, "declared_field_sentinel",
                    grain="row_and_cell_sentinel_only",
                )
            return
        ...
        window = self._declared_variance_window(entry, target)
        if grain.startswith("segment_"):
            variance_mask = self._segment_variance(rule, entry, target, base, window)
        else:
            variance_mask = self._entity_variance(rule, entry, target, base, window)
    ...
    def not_applicable(self, rule, entry, target):
        ...
        if entry_id.endswith("lgd_no_realisation"):
            ...
            specification = self.declarations.get("panel_scope_exclusions") or {}
            predicate = specification.get("row_predicate") if isinstance(specification, Mapping) else None
            if not isinstance(predicate, Mapping):
                self.plan[-1]["routing_status"] = "UNSCOPED"
                self.plan[-1]["missing_declarations"] = "panel_scope_exclusions.row_predicate"
                self.add_unscoped(rule, entry, target, [], ["panel_scope_exclusions.row_predicate"])
                return
            column = str(predicate.get("column") or "")
            operator = str(predicate.get("operator") or "in").lower()
            if column not in self.data or operator not in {"in", "not_in", "equals", "not_equals"}:
                self.plan[-1]["routing_status"] = "UNSCOPED"
                self.plan[-1]["missing_declarations"] = "panel_scope_exclusions.valid_row_predicate"
                self.add_unscoped(rule, entry, target, [], ["panel_scope_exclusions.valid_row_predicate"])
                return
            values = predicate.get("values")
            values = values if isinstance(values, list) else [predicate.get("value")]
            allowed = {_norm(value) for value in values if value is not None}
            observed = self.data[column].map(_norm)
            mask = observed.isin(allowed)
...
def resolve_precedence(raw_claims: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in work.groupby(["row_reference", "input_variable"], sort=False):
        winning_precedence = group["_precedence"].min()
        tied_tags = set(group.loc[group["_precedence"].eq(winning_precedence), "tag"])
        if len(tied_tags) > 1:
            raise ValueError("equal-precedence value-semantics claims require human review")
