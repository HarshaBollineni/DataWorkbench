...
def test_zero_llm_usage_is_explicit_and_validated(isolated_run):
    ...
    assert validated.events[0]["purpose"] == "semantic_role_verification"
...
def test_llm_audit_helpers_accept_a_diagnostic_specific_purpose(isolated_run):
    run_id, _ = isolated_run
    purpose = "expected_direction_adjudication"
    skipped = inference_audit.record_zero_llm_usage(
        run_id, actor="tester", purpose=purpose,
    )
    invoked = inference_audit.record_llm_call(
        run_id,
        status="succeeded",
        provider="azure_openai",
        model="direction-review-deployment",
        provider_api_version="2025-01-01-preview",
        prompt_template_id="t2d11-direction-review",
        prompt_template_version="1",
        prompt_hash="c" * 64,
        redacted_input_manifest={"feature": "debt_service_coverage"},
        validated_response={"decision": "MATCH"},
        source_artifact_references=[],
        proposed_mapping={"expected_direction": "DECREASING"},
        deterministic_mapping={"expected_direction": None},
        user_disposition="pending_manual_review",
        final_applied_mapping={"expected_direction": None},
        actor="tester",
        purpose=purpose,
    )
    ...
    assert skipped["purpose"] == purpose
    assert invoked["purpose"] == purpose
    assert {event["purpose"] for event in disclosure["events"]} == {purpose}
