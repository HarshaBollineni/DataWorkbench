...
from domains.aar.types import get_artifact_type, validate_payload
...
from domains.test_lab.diagnostics.t4_d14_population_stability.bins import create_draft_bins, validate_bin_definition
from domains.test_lab.diagnostics.t4_d14_population_stability.engine import calculate_feature_psi, classify
from domains.test_lab.diagnostics.t4_d14_population_stability.population import split_population
from domains.test_lab.diagnostics.t4_d14_population_stability import (
    manifest as manifest_population_stability,
    reporting as psi_reporting,
...
def test_artifact_types_are_registered_with_governed_semantics():
    ...
    assert get_artifact_type("population_stability_report").granularity == "run"
...
def test_one_snapshot_manifest_bin_review_runner_artifact_and_contextual_handoff():
    ...
    }, {"profile_json": service._column_profile(pd.Series(range(40)), [], True, "numerical")})
    persist_snapshot_profile_artifacts(item_id, actor="test")
    ...
    projected_feature = next(row for row in runner_population_stability.run_results(run_id)["results"]
                             if row["metrics_json"].get("result_kind") == "psi_feature")
    assert projected_feature["metrics_json"]["profile_artifact_id"]
    assert projected_feature["metrics_json"]["data_profile"]["total_count"] == 40
    ...
    assert report["features"][0]["review_state"] == "promoted"
    assert report["features"][0]["bins"][0]["contribution"] is not None
    assert report["features"][0]["binning_basis"]
    assert report["scope"]["split_criterion"] == "period <= 1"
    assert report["scope"]["baseline_count"] == report["scope"]["current_count"] == 20
    assert len(report["analysis_overview"]) == 4
    assert "applied unchanged to Current" in report["features"][0]["binning_basis"]
    assert report_artifact.artifact_type == "population_stability_report"
    assert reused is False
    text = runner_population_stability.report_text(run_id)
    assert "Each feature is divided into common bins" in text
    assert "Split criterion: period <= 1" in text
    assert "Baseline population: 20 rows (50.00%)" in text
    assert "Baseline feature profile" in text
    assert "Population profile" in text
    assert "-infinity" in text and "∞" not in text
    assert "------------------------------------------------------------" in text
    pdf, metadata = runner_population_stability.report_document(run_id)
    assert pdf.startswith(b"%PDF")
    assert metadata["filename"] == f"population-stability-{run_id}.pdf"
...
def test_report_bin_labels_use_pdf_safe_infinity_and_two_decimal_boundaries():
    assert psi_reporting._bin_label("(−∞, 5.9350000000000005]") == "(-infinity, 5.94]"
    assert psi_reporting._bin_label("(5.9350000000000005, +∞]") == "(5.94, +infinity]"
    assert psi_reporting._bin_label("A: retail") == "A: retail"
