...
from domains.test_lab.diagnostics.t2_d04_cross_field_business_rule import engine as cf_engine  # noqa: E402
from domains.test_lab.diagnostics.t2_d04_cross_field_business_rule import result as cf_result  # noqa: E402
from domains.test_lab.diagnostics.t2_d04_cross_field_business_rule import roles as cf_roles  # noqa: E402
from domains.test_lab.diagnostics.t2_d04_cross_field_business_rule import rules as cf_rules  # noqa: E402
...
CORE_DIR = BACKEND_ROOT / "domains" / "test_lab" / "diagnostics" / "t2_d04_cross_field_business_rule"
...
class PurityTests(unittest.TestCase):
    ...
    def test_core_imports_no_model_provider(self):
        ...
        for path in (
            BACKEND_ROOT / "dq_diagnostics" / "readiness.py",
            CORE_DIR / "manifest.py",
            CORE_DIR / "runner.py",
        ):
            source = path.read_text(encoding="utf-8")
            self.assertIsNone(banned.search(source), f"{path.name} imports a model provider")
    ...
    def test_no_codegen_on_the_new_run_path(self):
        ...
        offenders = []
        for root in ("dq_diagnostics", "domains/test_lab/diagnostics/t2_d04_cross_field_business_rule", "ingest"):
    ...
    def test_no_snippet_or_sandbox_mechanism_on_the_new_run_path(self):
        for root in ("dq_diagnostics", "domains/test_lab/diagnostics/t2_d04_cross_field_business_rule"):
            for path in (BACKEND_ROOT / root).rglob("*.py"):
                source = path.read_text(encoding="utf-8")
