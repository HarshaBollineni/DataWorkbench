...
from dq_diagnostics import runner_cross_field as runner  # noqa: E402
...
class NoKnowledgeBaseTests(unittest.TestCase):
    ...
    def test_an_empty_rule_set_verdicts_as_not_applicable_never_pass(self):
        from dq_diagnostics.engines.cross_field.result import StructuredResult
...
class EndToEndRunTests(unittest.TestCase):
    ...
    def test_coverage_summary_names_covered_and_pending_without_a_health_score(self):
        ...
        self.assertEqual(summary["executable_total"], 4)
        self.assertEqual([d["diagnostic_id"] for d in summary["executable"]], [2, 4, 6, 14])
        self.assertEqual(len(summary["workflow_pending"]), 5)
...
class CoverageBoardTests(unittest.TestCase):
    ...
    def test_each_card_exposes_latest_completed_and_full_immutable_run_history(self):
        ...
        for index, status in enumerate(("done", "failed", "draft")):
            ...
            s.insert("diag_runs", {"run_id": run_id, "item_id": item_id, "diagnostic_id": 4,
            ...
            if status != "draft":
                s.insert("diag_results", {"result_id": f"dres_{uuid.uuid4().hex[:10]}",
                    "run_id": run_id, "diagnostic_id": 4, "entity_or_table": "portfolio",
        ...
        card = next(row for row in board["cards"] if row["diagnostic_id"] == 4)
        ...
        history = v2.diagnostic_run_history(item_id, 4)
    ...
    def test_a_manifest_for_a_pending_diagnostic_is_refused_with_the_exact_message(self):
        with self.assertRaises(HTTPException) as ctx:
            v2.build_diagnostic_manifest(self.item_id, v2.ManifestIn(diagnostic_id=8))
    ...
    def test_a_run_request_for_a_pending_diagnostic_is_refused(self):
        ...
        execute."""
        run_id = "drun_pending_fixture"
        s.insert("diag_runs", {
            "run_id": run_id, "item_id": self.item_id, "diagnostic_id": 8,
