...
| 4 | T2 | hard | Cross-field business rule | Does each row satisfy a hard cross-field rule (with exceptions)? | violation count / rate | No - deterministic rule | violation rate > ~0 after exceptions → flag | Deterministic | verdict | Both | HIGH | **executable** | D-15 (30 Jul 2026) |
...
| 8 | T2 | hard | Value-semantics classification | Classify each field/null as valid / missing / censored / stale / N-A | rule-based classification vs KB | No - classification | tag per KB rule (no cutoff) — OUTPUT other tests consume | Deterministic | classification output | Stage 1 | HIGH | workflow_pending (next to enable — DIA-01) | — |
