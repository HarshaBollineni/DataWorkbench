...
| DX-05 | The three retained PSI parity spikes (`spike_recalibrate.py`, `spike_gx.py`, `spike_integration.py`) are ported: each uses deterministic spike-only `backend/spike_fixtures.py` and exits 0. | They remain parity evidence until later D-22-approved PSI enablement; the completed port enables no B2 diagnostic. | 31 Jul 2026 |
