...
function OrientationToggle({ value, busy, onChange }) {
  return <div>
    ...
    <div className="mt-1 grid max-w-md grid-cols-2 rounded-lg bg-slate-100 p-1" role="group" aria-label="Target risk direction">
...
function AiSuggestionSummary({ feature, changed, busy, onRestore }) {
  ...
  return <div className="mb-3 rounded border border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-950">
    ...
    <div className="mt-2 rounded border border-indigo-100 bg-white/80 px-3 py-2">
      ...
      {matched && linkedCandidate?.knowledge_rule_id && <p className="mt-1 font-semibold text-indigo-800">Rule {linkedCandidate.knowledge_rule_id}</p>}
...
function ReviewRow({ feature, busy, patch, expanded, onToggle }) {
  ...
  const kbVersion = feature.governed_exact_decision?.kb_version || "active";
  ...
  return <article className="border-b border-slate-100 last:border-0" data-testid="directionality-feature-card">
    <div className="grid items-center gap-3 px-4 py-3 md:grid-cols-[minmax(0,1.15fr)_minmax(11rem,0.7fr)_minmax(13rem,1fr)_auto]">
      ...
      <div className="min-w-0"><span className="block text-[10px] font-semibold uppercase tracking-wide text-slate-400">{suggestion.label}</span><strong className="mt-0.5 block break-words text-xs text-slate-800" title={suggestion.conceptId ? `KB concept: ${suggestion.conceptId}` : undefined}>{feature.knowledge_rule_id ? `${feature.knowledge_rule_id} — ` : ""}{suggestion.concept}</strong>{feature.expected_direction && <span className={`mt-1 block text-[11px] font-medium ${needsReview ? "text-amber-700" : "text-slate-500"}`}>{DIRECTION_LABELS[feature.expected_direction]}</span>}</div>
    ...
    {expanded && <div className="border-t border-slate-100 bg-slate-50 px-4 py-4">
      ...
      {!!feature.candidates?.length && <details className="mb-3 rounded border border-amber-200 bg-white p-2">
        <summary className="cursor-pointer text-xs font-medium text-amber-900">Review possible KB concepts ({feature.candidates.length})</summary>
        <p className="mt-1 text-[11px] text-amber-800">{CAUTION}</p>
        <div className="mt-2 grid gap-1 lg:grid-cols-2">{candidates.map((item) => <button type="button" key={item.knowledge_rule_id || item.canonical_feature} disabled={busy} onClick={() => chooseCandidate(item)} className={`rounded border px-2 py-1 text-left text-xs disabled:cursor-not-allowed disabled:opacity-60 ${candidate === item.canonical_feature ? "border-indigo-400 bg-indigo-50" : "border-slate-200 bg-white"}`}><strong>{item.knowledge_rule_id ? `${item.knowledge_rule_id} — ` : ""}{item.canonical_feature}</strong><span className="ml-2 text-slate-500">text score {item.combined_similarity_score}</span><small className="block text-slate-500">Matched text: {item.best_matching_kb_text}</small>{item.rationale && <small className="mt-1 block text-slate-600">Direction rationale: {item.rationale}</small>}</button>)}</div>
        {feature.candidate_display_limit < feature.candidates.length && <Button className="mt-2" size="sm" variant="outline" disabled={busy} onClick={() => patch({ kind: "candidate_display", feature: feature.feature, limit: feature.candidate_display_limit + 3 })}><ChevronDown className="h-3.5 w-3.5" /> Show 3 more</Button>}
      </details>}
...
export default function DirectionalityScopeGate({ manifest, busy, patch, runNow }) {
  ...
  return <section className="grid gap-4" data-testid="directionality-scope-gate">
    ...
    <Step number="1" title="Confirm the reference" complete={referenceReady}>
      <p className="mb-3 text-xs text-slate-500">Confirm the target and whether a higher target value means higher or lower risk.</p>
      <div className="overflow-hidden rounded-lg border border-slate-200">
        <section className="p-3">
          ...
          <details className="mt-3 rounded border border-slate-200 bg-slate-50 px-3 py-2"><summary className="cursor-pointer text-xs font-medium text-slate-700">Advanced target settings</summary><div className="mt-2 flex flex-wrap gap-2"><select aria-label="Target type" value={targetType} onChange={(event) => setTargetType(event.target.value)} className="h-9 rounded border border-slate-300 bg-white px-2 text-xs"><option value="auto">Detect automatically</option><option value="binary">Binary</option><option value="continuous">Continuous</option></select><input aria-label="Positive event class" className="h-9 w-36 rounded border border-slate-300 bg-white px-2 text-xs" value={positiveClass} onChange={(event) => setPositiveClass(event.target.value)} placeholder="Positive event class" /><Button size="sm" variant="outline" disabled={busy} onClick={() => patch({ kind: "target_config", target_type: targetType, positive_class: positiveClass || null })}>Save</Button></div></details>
        </section>
        <section className="border-t border-slate-200 bg-slate-50/60 p-3">
          <header className="mb-3 flex flex-wrap items-start justify-between gap-2"><div><div className="flex items-center gap-2"><GitBranch className="h-4 w-4 text-slate-500" /><h3 className="text-sm font-semibold text-slate-900">Analysis view</h3></div><p className="mt-0.5 text-xs text-slate-500">{segmentAnalysisAvailable ? "Keep the overall view or add one segment to this rerun." : "The first run assesses the overall portfolio. A later run can add one segment."}</p></div>{!manifest.segment_column && <Badge variant="secondary">Overall portfolio</Badge>}</header>
          {!segmentAnalysisAvailable ? <p className="text-xs text-slate-500">Complete the overall analysis once to make segment selection available.</p> : !segmentCandidates.length ? <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2"><div><strong className="text-xs text-amber-900">No eligible split fields found</strong><span className="ml-2 text-xs text-amber-800">A non-constant field other than the target or an identifier is required.</span></div><Badge variant="secondary">Overall portfolio</Badge></div> : <div className="grid gap-3">
    ...
    <Step number="2" title="Select numeric features" complete={savedScope.length > 0}>
      {!referenceReady && <p className="mb-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900"><strong>Target risk direction is still required.</strong> You can select features now, then confirm the direction above before continuing.</p>}
        <div className="mb-3 text-xs leading-5 text-slate-600">Only numeric or meaningfully ordered features can be assessed.
        ...
        <div className="max-h-[31rem] overflow-y-auto rounded-lg border border-slate-200 bg-slate-50 p-3">{selectableFeatures.length ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{selectableFeatures.map((feature) => <ScopeFeatureCard key={feature.feature} feature={feature} checked={selected.includes(feature.feature)} disabled={busy} onToggle={() => toggleFeature(feature.feature)} />)}</div> : <p className="p-4 text-center text-xs text-slate-500">No eligible numeric features are available for this target.</p>}</div>
        <div className="mt-3 flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-3"><div><strong className="text-sm text-slate-900">{selected.length} feature(s) selected</strong><p className="text-xs text-slate-500">{referenceReady ? "Continue to resolve the expected economic direction for each feature." : "Confirm the target risk direction above to continue."}</p></div><Button disabled={busy || !selected.length || !referenceReady} onClick={continueToReview}>Resolve expected directions <ChevronRight className="h-4 w-4" /></Button></div>
    </Step>
  </section>;
}
