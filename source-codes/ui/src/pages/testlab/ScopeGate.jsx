...
import FeatureTargetScopeGate from "./FeatureTargetScopeGate";
import PopulationStabilityScopeGate from "./PopulationStabilityScopeGate";
import RowCompletenessScopeGate from "./RowCompletenessScopeGate";
...
export default function ScopeGate({ runId, onRunStarted }) {
  ...
  const reload = useCallback(() => getDiagnosticManifestV2(runId).then((data) => {
      ...
      setError("");
    }).catch((e) => { setError(e.message); throw e; }), [runId]);
  ...
  const acceptManifest = (updatedManifest) => {
  ...
  if (manifest.manifest_kind === "feature_target_separation") {
    return <div className="grid gap-4">{diagnosticError}<FeatureTargetScopeGate run={run} manifest={manifest} busy={busy}
      patch={patch} runNow={runNow} /></div>;
  }
  if (manifest.manifest_kind === "population_stability_index") {
    return <div className="grid gap-4">{diagnosticError}<PopulationStabilityScopeGate run={run} manifest={manifest} busy={busy}
      patch={patch} runNow={runNow} reload={reload} acceptManifest={acceptManifest} /></div>;
  }
