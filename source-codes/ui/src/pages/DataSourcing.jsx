...
import StagedStructureReview from "@/pages/sourcing/StagedStructureReview.jsx";
...
function UploadFlow({ kind, onBack, onTimeout, onStartFresh, onDraftConflict, onDraftIdentified, onReturnToColumns, initialAsset = null }) {
  ...
  const [structuralPrecheckLoading, setStructuralPrecheckLoading] = useState(false);
  const [structureReviewReady, setStructureReviewReady] = useState(false);
  const inventoryRef = useRef(null);
  ...
  const processDisabledReasons = [
    ...
    sourceComplete && !structureReviewReady && "save the staged Dataset Structure review in Step 5",
  ...
  useEffect(() => {
    if (!itemId || !inventoryRows.length || completion) {
      ...
      setStructuralPrecheckLoading(false);
      setStructureReviewReady(false);
      return undefined;
    }
    let live = true;
    setStructuralPrecheckLoading(true);
    setStructureReviewReady(false);
    const timer = window.setTimeout(() => {
      getStructuralPrecheckV2(itemId, inventoryRows)
        ...
        .finally(() => { if (live) setStructuralPrecheckLoading(false); });
    }, 150);
  ...
  return (
    <section>
      <details id="sourcing-step-1" open={sourceOpen} onToggle={(event) => setSourceOpen(event.currentTarget.open)} className="scroll-mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      {sourceComplete && <StepCard anchor="sourcing-step-4" step="4" title="Confirm dataset information" subtitle="Dictionary and profiling evidence preloads the target, period and business context." testId="upl-step-4">
    ...
    <main className="min-h-screen bg-slate-50 p-5 md:p-6">
      ...
      {!kind ? (
        ...
        <UploadFlow key={`${kind}:${startAsset?.resume_snapshot_id || startAsset?.asset_id || "new"}`} kind={kind} initialAsset={startAsset} onBack={backToKinds} onTimeout={handleTimeout} onStartFresh={startFresh} onDraftConflict={handleDraftConflict}
          onDraftIdentified={(snapshotId) => updateLocation({ resume: snapshotId, new: null, browse: null })} onReturnToColumns={returnToColumnDefinitions} />
      )}
    </main>
  );
}
